...
export default function Footer({
  ...
  return (
    <footer className="w-full bg-dark-900 text-light-100">
      <div className="mx-auto max-w-7xl px-4 pb-8 pt-10 md:px-6 lg:px-8">
        <div className="grid grid-cols-2 gap-8 pb-8 md:grid-cols-4 lg:grid-cols-6">
          ...
          {sections.map((section, index) => (
            <div key={section.title ?? `links-${index}`} className="space-y-4">
              {section.title && <h4 className="text-body ">{section.title}</h4>}
          ...
          <div className="col-span-2 flex items-start gap-3 md:col-span-1 md:justify-end lg:col-span-1">
            {socialLinks.map((social) => (
              <Link
                key={social.label}
                href={social.href}
                aria-label={social.label}
                className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-light-100 transition-colors hover:bg-[var(--color-light-100)]/10"
              >
        ...
        <div className="flex flex-col gap-4 border-t border-[color-mix(in_srgb,var(--color-light-100)_10%,transparent)] pt-4 text-caption text-dark-500 md:flex-row md:items-center md:justify-between">
